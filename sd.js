  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      if (x + 1 < w) {
        ...
        effectRGB = palette.diffusion(calcRGB, errRGB, 8.0 / 42);
        palette.setRGB(x + 1, y, pixels, effectRGB);
      }

      if (x + 2 < w) {
        calcRGB = palette.getRGB(x + 2, y, pixels);
        effectRGB = palette.diffusion(calcRGB, errRGB, 4.0 / 42);
        palette.setRGB(x + 2, y, pixels, effectRGB);
      }

      if (x - 2 >= 0 && y + 1 < h) {
        calcRGB = palette.getRGB(x - 2, y + 1, pixels);
        effectRGB = palette.diffusion(calcRGB, errRGB, 2.0 / 42);
      ...
      if (x - 2 >= 0 && y + 2 < h) {
        ...
        effectRGB = palette.diffusion(calcRGB, errRGB, 1.0 / 42);
